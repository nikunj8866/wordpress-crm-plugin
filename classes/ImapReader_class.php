...
class ImapReader_class
{
...

    {
        ...
        $mail->uid = $this->_imap_uid($this->box, $id);

        $mail->uid = $this->_imap_uid($this->box, $id);

        $mail->message_id = $header->message_id;

        $mail->in_reply_to =$header->in_reply_to;

        return $mail;

    }

    private function _imap_uid($mbox, $id)
    {
        return imap_uid($mbox, $id);
    }
    ...
    public function _get_from_email($string, $start = '<', $end = '>')
    {
        $string = ' ' . $string;
    ...
    public function search_greater_uid($last_id)
    {
        $results = imap_fetch_overview($this->box, $last_id.":*", FT_UID);
        if ($results)
        {
            $this->list = array ();
            foreach ($results as $result)
            {
                $this->list[] = $this->_fetchHeader($mbox, $result->msgno);
            }
            return true;
        }
        return false;
