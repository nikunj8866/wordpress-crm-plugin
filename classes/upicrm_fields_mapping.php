...
if (!class_exists('UpiCRMFieldsMapping')) {
    ...
    class UpiCRMFieldsMapping extends WP_Widget {
        ...
        function empty_form_by_id($source_type, $source_id) {
            //delete fields mapping by 
            $this->wpdb->delete(upicrm_db() . "fields_mapping", array('source_type' => $source_type, "source_id" => $source_id));
        }
        //Field id 8 is a email field of all forms
        function get_form_field_id($source_id, $field_id = 8)
        {
            $rows = $this->wpdb->get_results("SELECT fm_name FROM `" . upicrm_db() . "fields_mapping` WHERE `source_id` = '{$source_id}' AND `field_id` = {$field_id}");
            if($rows)
            {
                return $rows[0]->fm_name;
            }
            return false;
        }
